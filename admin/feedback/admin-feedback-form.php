	...
	public function show_deactivate_feedback_popup() {
		...
		$deactivate_reasons = array(
			'didnt_work_as_expected'         => array(
				'title'             => __( 'The plugin didn\'t work as expected.', 'contact-form-extender-for-divi-builder' ),
				'input_placeholder' => 'What did you expect?',
			),
			'found_a_better_plugin'          => array(
				'title'             => __( 'I found a better plugin.', 'contact-form-extender-for-divi-builder' ),
				'input_placeholder' => __( 'Please share which plugin.', 'contact-form-extender-for-divi-builder' ),
			),
			'couldnt_get_the_plugin_to_work' => array(
				'title'             => __( 'The plugin is not working.', 'contact-form-extender-for-divi-builder' ),
				'input_placeholder' => 'Please share your issue. So we can fix that for other users.',
			),
			'temporary_deactivation'         => array(
				'title'             => __( 'It\'s a temporary deactivation.', 'contact-form-extender-for-divi-builder' ),
				'input_placeholder' => '',
			),
			'other'                          => array(
				'title'             => __( 'Other reason.', 'contact-form-extender-for-divi-builder' ),
				'input_placeholder' => __( 'Please share the reason.', 'contact-form-extender-for-divi-builder' ),
			),
		);
		...
		<div id="cool-plugins-feedback-<?php echo esc_attr( $this->plugin_slug ); ?>" class="hide-feedback-popup">
			...
			<div class="cp-feedback-header">
				<div class="cp-feedback-title"><?php echo esc_html__( 'Quick Feedback', 'contact-form-extender-for-divi-builder' ); ?></div>
			...
			<div class="cp-feedback-form-wrapper">
				<div class="cp-feedback-form-title"><?php echo esc_html__( 'If you have a moment, please share the reason for deactivating this plugin.', 'contact-form-extender-for-divi-builder' ); ?></div>
	...
	function submit_deactivation_response() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), '_cool-plugins_deactivate_feedback_nonce' ) || ! current_user_can( 'manage_options' )) {
			...
			$deactivate_reasons = array(
				...
				'other'                          => array(
					'title'             => __( 'Other', 'contact-form-extender-for-divi-builder' ),
					'input_placeholder' => __( 'Please share the reason.', 'contact-form-extender-for-divi-builder' ),
				),
			);

			$plugin_initial =  get_option( 'cfefd_initial_version' );
			...
			$feedback_url      = CFEFD_FEEDBACK_URL.'wp-json/coolplugins-feedback/v1/feedback';
			$install_date 		= get_option('cfefd-install-date');
			$unique_key     	= '12';
			...
			$response          = wp_remote_post(
				...
				array(
                    'timeout' => 30,
                        'body'    => array(
                        'server_info' => wp_json_encode($this->cfefd_get_user_info()['server_info']),
                        'extra_details' => wp_json_encode($this->cfefd_get_user_info()['extra_details']),
