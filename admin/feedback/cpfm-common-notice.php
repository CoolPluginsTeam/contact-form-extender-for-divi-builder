...
class CPFM_Feedback_Notice {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_cpfm_handle_opt_in_cfefd', [ $this, 'cpfm_handle_opt_in_choice' ]);
    ...
    public function cpfm_enqueue_assets() {
        ...
        wp_enqueue_style( 'cpfm-common-review-style', CFEFD_PLUGIN_URL . 'admin/feedback/css/cpfm-admin-feedback.css', array(), CFEFD_PLUGIN_VERSION );


        wp_enqueue_script(
            'cpfm-common-review-script', 
            CFEFD_PLUGIN_URL . 'admin/feedback/js/cpfm-admin-feedback.js', 
            ['jquery'], 
    ...
    public function cpfm_render_notice_panel() {
        ...
        $output .= '<div class="notice-panel-header">' . esc_html__('Help Improve Plugins', 'contact-form-extender-for-divi-builder') . ' <span class="dashicons dashicons-no" id="cpfm_remove_notice"></span></div>';
        ...
        foreach (self::$registered_notices as $key => $notice) {
            ...
            $output .= '<p>' . esc_html($notice['message']) . '<a href="#" class="cpf-toggle-extra">' . esc_html__(' More info', 'contact-form-extender-for-divi-builder') . '</a></p>';
            $output .= '</div>';
            ...
            $output .= '<button class="button button-primary opt-in-yes" data-category="' . esc_attr($key) . '" id="yes-share-data" value="yes">' . esc_html__("Yes, I Agree", 'contact-form-extender-for-divi-builder') . '</button>';
            $output .= '<button class="button opt-in-no" data-category="' . esc_attr($key) . '" id="no-share-data" value="no">' . esc_html__('No, Thanks', 'contact-form-extender-for-divi-builder') . '</button>';
            $output .= '</div>';
